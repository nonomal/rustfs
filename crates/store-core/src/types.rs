...
use crate::bucket_replication::{ReplicationStatusType, VersionPurgeStatusType};
use crate::error::{Error, Result};
use crate::heal::{HealOpts, HealResultItem, HealSequence};
use crate::lifecycle::{ExpirationOptions, LcAuditEvent, TransitionOptions, TransitionedObject};
...
// pub type DiskStore = Box<dyn DiskAPI + Send + Sync + 'static>;
...
impl ObjectInfo {
    ...
    pub fn from_file_info(fi: &FileInfo, bucket: &str, object: &str, versioned: bool) -> ObjectInfo {
        ...
        let metadata = {
            ...
            for key in ["md5Sum", "etag", "expires", "X-Amz-Tagging", "last-modified", "STANDARD"] {
                v.remove(key);
            }
            v
        };
